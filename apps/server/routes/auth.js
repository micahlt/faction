...
export default function authRouter(prisma) {
    ...
    router.post("/signup", async (req, res) => {
        ...
        if (password.match(PASSWORD_REGEX).length != 1) {
            return res.status(400).send({
                error: "You must use a password that is 8 to 64 characters long and contains a mix of upper and lower case characters, one number and one special character.",
            });
        }
